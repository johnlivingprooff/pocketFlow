...
import { View, Text, Modal, TouchableOpacity, TextInput, ScrollView, KeyboardAvoidingView, Platform, useColorScheme } from 'react-native';
...
export function TransferModal({ visible, onClose, wallets, balances, onTransfer, themeMode = 'light' }: TransferModalProps) {
  ...
  return (
    <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}>
      ...
      >
        <View style={{ backgroundColor: t.background, borderTopLeftRadius: 24, borderTopRightRadius: 24, maxHeight: '90%', flexShrink: 1 }}>
          <ScrollView
          ...
          >
            <View style={{ padding: 20 }}>
              ...
              {/* Error - reserve space always to prevent layout shift */}
              <View style={{ minHeight: error ? 'auto' : 0, marginBottom: error ? 16 : 0 }}>
                {error ? (
                  <View style={{ backgroundColor: '#FEE', borderRadius: 8, padding: 12 }}>
                    <Text style={{ color: '#C00', fontSize: 13 }}>{error}</Text>
                  </View>
                ) : null}
              </View>

              {/* Transfer Summary - always reserve space */}
              <View style={{ minHeight: fromWallet && toWallet && amount && !isNaN(parseFloat(amount)) && parseFloat(amount) > 0 ? 'auto' : 0, marginBottom: fromWallet && toWallet && amount && !isNaN(parseFloat(amount)) && parseFloat(amount) > 0 ? 16 : 0 }}>
                {fromWallet && toWallet && amount && !isNaN(parseFloat(amount)) && parseFloat(amount) > 0 && (
                  <View style={{ backgroundColor: t.card, borderRadius: 12, padding: 14 }}>
